...
function freezeExpectedDiagnostic(item) {
  ...
  return Object.freeze({
    code: item.code,
    scoreEventId: item.scoreEventId ?? null,
    midiEventId: item.midiEventId ?? null,
  })
}
...
function diagnosticKey(item) {
  return JSON.stringify([
    item.code,
    item.scoreEventId ?? item.location?.eventId ?? item.details?.scoreEventId ?? null,
    item.midiEventId ?? item.details?.midiEventId ?? null,
  ])
}
...
export function createMidiEvaluationCase({
  id,
  sourceType,
  oracleType,
  input,
...
function evaluateCase(expectedDiagnostics, actualDiagnostics) {
  ...
  const actual = new Set((actualDiagnostics ?? [])
    .filter((item) => DIAGNOSTIC_CODES.includes(item?.code))
    .map(diagnosticKey))
  ...
  return Object.freeze({
    ...
    recall: metricRatio(truePositive, truePositive + falseNegative),
    exactLabelSetMatch: falsePositive === 0 && falseNegative === 0,
  })
}
...
export async function runMidiEvidenceBenchmark(cases, analyze) {
  ...
  for (const benchmarkCase of cases) {
    ...
    results.push(Object.freeze({
      ...
      oracleType: benchmarkCase.oracleType,
      alignmentStatus,
      alignmentConfidence: Number.isFinite(result?.alignment?.confidence) ? result.alignment.confidence : null,
      ...labelMetrics,
    }))
  }
  ...
  return Object.freeze({
    ...
    bySourceType: Object.freeze(Object.fromEntries([...bySourceType.entries()].map(([key, value]) => [key, finalizeAggregate(value)]))),
