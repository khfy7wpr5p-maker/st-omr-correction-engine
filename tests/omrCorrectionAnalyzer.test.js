...
test('end-to-end analyzer emits bounded shadow proposals for pitch, duration, onset and staff mismatches', () => {
  const event = createScoreEvent({
    ...
    metadata: Object.freeze({
      expectedPitch: 60,
      expectedDurationQuarterBeats: 1,
      expectedOnsetQuarterBeats: 0,
      expectedStaff: 1,
    }),
  })
...
test('all deterministic structural proposals can be projected, independently revalidated and reverted without source mutation', () => {
  ...
  const patches = analyzed.suggestions
    .filter((suggestion) => suggestion.errorClass !== 'VOICE')
    .flatMap((suggestion) => suggestion.proposedPatches)
...
test('tie detector reads Audiveris importer tieTypes and proposes the uniquely missing counterpart', () => {
  const start = createScoreEvent({
    ...
    staff: 1,
    pitch: 60,
    metadata: Object.freeze({ sourceOrder: 1, tieTypes: Object.freeze(['start']) }),
  })
  const target = createScoreEvent({
    id: 'n2',
    measureKey: 'm1',
    onset: 1,
    duration: 1,
    ...
    staff: 1,
    pitch: 60,
    metadata: Object.freeze({ sourceOrder: 2 }),
  })
  ...
  assert.equal(suggestion.operation, PATCH_OPERATION.CHANGE_RELATION)
  assert.equal(suggestion.eventId, 'n2')
  assert.deepEqual(suggestion.after.tieTypes, ['stop'])
  ...
  const revalidated = revalidateProjectedRevisionV2({
    sourceGraph: source,
    projectedGraph: projected.graph,
    patches: suggestion.proposedPatches,
  })
  assert.equal(revalidated.decision, 'PASS')
})
...
test('tie anomalies abstain instead of guessing when no unique counterpart exists', () => {
  const source = graphWith([createScoreEvent({
    ...
    staff: 1,
    pitch: 60,
    metadata: Object.freeze({ tieTypes: Object.freeze(['start']) }),
  })])
